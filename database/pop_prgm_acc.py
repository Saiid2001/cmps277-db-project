...
def pop_prgrm_acc(mydb):
	mycursor = mydb.cursor()

	mycursor.execute("Select id from Complete_program")
	...
	for prg in programs:
		...
		for i in range(num):
			...
			if acc not in accs:
				accs.append(acc)
				mySql_insert_query = "INSERT INTO prg_accomplishments VALUES (" + str(prg[0]) + ",'" + acc + "','" + content + "')"
				cursor = mydb.cursor()
	...
	print(count, "Record inserted successfully into prg_accomplishments table")
